    ...
    def _convert_audio(self, input_path: Path, output_path: Path):
        ...
        try:
            ...
            if input_path.suffix.lower() != ".wav":
                console.print(f"[red]✗ pydub 未安装，无法将 {input_path.suffix} 转为 wav，声音克隆将不可用[/red]")
                console.print("[red]  请运行: pip install pydub[/red]")
                raise
            shutil.copy2(input_path, output_path)
        except Exception as e:
            console.print(f"[yellow]⚠ 音频转换失败 ({e})，尝试直接拷贝[/yellow]")
            if input_path.suffix.lower() != ".wav":
                console.print(f"[red]✗ 非 wav 文件无法直接使用，跳过 {input_path.name}[/red]")
                raise
            shutil.copy2(input_path, output_path)
...
class GPTSoVITSEngine(VoiceEngine):
    ...
    def _convert_audio(self, input_path: Path, output_path: Path):
        ...
        try:
            ...
            if input_path.suffix.lower() != ".wav":
                console.print(f"[red]✗ pydub 未安装，无法将 {input_path.suffix} 转为 wav[/red]")
                raise
            shutil.copy2(input_path, output_path)
        except Exception as e:
            console.print(f"[yellow]⚠ 音频转换失败 ({e})，尝试直接拷贝[/yellow]")
